            ...
            if ($objValid->isValid()) {
                if ($objCatalogue->updateProduct($objValid->_post, $id)) {
                    ...
                    if ($objUpload->upload(CATALOGUE_PATH)) {
                        ...
                        $objCatalogue->updateProduct(array(
                            'image' => $objUpload->_names[0]
                                ), $id);
                        Helper::redirect('/admin' . Url::getCurrentUrl(array(
                                    'action', 'id'
                                )) . '&action=edited');
                    } else {
                        Helper::redirect('/admin' . Url::getCurrentUrl(array(
                        ...
                        <select name="category" id="category" class="sel">
                            ...
                            <?php if (!empty($categories)) { ?>
                                <?php foreach ($categories as $cat) { ?>
                                    ...
                                    <?php 
                                    echo $objForm->stickySelect('category', $cat['id'], $product['category']); ?>                          
                                            >
