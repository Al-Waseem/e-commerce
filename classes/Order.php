...
class Order extends Application {
    ...
    public function getItems() {
        ...
        if (!empty($this->_basket)) {
            $objCatalouge = new Catalogue();
            foreach ($this->_basket as $key => $value) {
                $this->_items[$key] = $objCatalouge->getProduct($key);
            }
        }
    }
    ...
    public function addItem($order_id = NULL) {
        if (!empty($order_id)) {
            ...
            foreach ($this->_items as $item) {
                $sql = "INSERT INTO `{$this->_table_2}`
                        ...
                        . $item['price'] . "', '" . $this->_basket[$item['id']]['qty'] . "')";
    ...
    public function getOrderItems($id = null) {
        ...
        $sql = "SELECT * FROM `{$this->_table_2}` WHERE `order` = '" . $this->db->escape($id) . "'";

        return $this->db->fetchAll($sql);
    }

    //hemos cancelado este metodo
    /*
      public function approve($txn_id = null, $payment = null, $id = null) {
      if (!empty($txn_id) && !empty($payment) && !empty($id)) {
    ...
    public function approve($array = null, $result) {
